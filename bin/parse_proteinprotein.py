...
def filterBlastXML(path):
    ...
    for (queryDef, hitId, hitLen, eValue) in zip(queryDefs, hitIds, hitLens, eValues):
        ...
        if parts and parts[0] == args.speciesId:
            ...
            elif hitLen > ret[queryDef][1] or (hitLen == ret[queryDef][1] and eValue > ret[queryDef][2]):
                ret[queryDef] = (hitId, hitLen, eValue)
            elif hitLen == ret[queryDef][1] and eValue == ret[queryDef][2]:
                raise RuntimeError("ERROR: Blast rows found with identical alignment length and e-value!")
    return set((r[0] for r in ret.values()))


def findMatches(protein):
    """
    Getter method.
...
def processProteinLinks(graph, rows):
    """
