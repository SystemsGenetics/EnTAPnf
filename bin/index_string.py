...
def populateInfoDB(conn, ifile):
    ...
    while True:
        ...
        if not line:
            conn.cursor().execute(
                "CREATE INDEX idx_protein_info_protein_external_id" " ON protein_info (protein_external_id);"
            )
            conn.commit()
            break
        parts = line[:-1].split("\t")
...
def populateLinksDB(conn, ifile):
    """
